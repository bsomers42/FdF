...
/*   Updated: 2022/06/10 14:11:43 by bsomers       ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */

#include "FdF.h"

void	calc_coord(t_draw *draw, int z, t_br *br, mlx_image_t *g_img)
{
	int	prev_x;
	int	prev_y;
...
void	coord_and_horiz_lines(t_draw *draw, t_map *map, t_br *br, \
mlx_image_t *g_img)
{
	int	i;
	int	j;
	...
	while (i < map->y)
	{
		while (j < map->x)
		{
			draw->x = 700 + j * 14 * (0.1 * draw->zoom) + br->move_x;
			draw->y = -200 + i * 14 * (0.1 * draw->zoom) + br->move_y;
			calc_coord(draw, map->map[i][j], br, g_img);
...
void	vert_lines(t_draw *draw, t_map *map, t_br *br, mlx_image_t *g_img)
{
	int	i;
	int	j;
